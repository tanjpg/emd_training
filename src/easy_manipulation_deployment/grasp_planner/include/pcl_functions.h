...
namespace PCLFunctions
{
  ...
  void centerCamera(pcl::PointCloud<pcl::PointXYZRGB>::Ptr target_cloud, pcl::visualization::PCLVisualizer::Ptr viewer)
  {
  ...
  void viewCloud(pcl::PointCloud<pcl::PointNormal>::Ptr target_cloud, pcl::visualization::PCLVisualizer::Ptr viewer)
  {
  ...
  void viewerAddNormalCloud(pcl::PointCloud<pcl::PointNormal>::Ptr target_ncloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr target_cloud, std::string name, pcl::visualization::PCLVisualizer::Ptr viewer)
  {
    std::vector<int> nanNormalIndices;
    pcl::removeNaNNormalsFromPointCloud(*target_ncloud, *target_ncloud, nanNormalIndices);
    viewer->addPointCloudNormals<pcl::PointXYZRGB, pcl::PointNormal> (target_cloud, target_ncloud, 10, 0.05, name);
    viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 1.0, 1.0, name); 
  }

  void viewerAddRGBCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr target_cloud, std::string name, pcl::visualization::PCLVisualizer::Ptr viewer)
  {
  ...
  bool passthroughFilter(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
  ...
  void SensorMsgtoPCLPointCloud2(const sensor_msgs::msg::PointCloud2 &pc2, pcl::PCLPointCloud2 &pcl_pc2)
  {
    try { /* */  
  ...
  bool planeSegmentation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_plane_removed, 
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_table, int segmentation_max_iterations, float segmentation_distance_threshold)
  {
  ...
  void removeAllZeros(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud)
  {
    ...
    for(auto i = temp_cloud.points.begin(); i != temp_cloud.points.end(); i++)
    {
      if(std::abs((*i).x) < 0.0001 && std::abs((*i).y) < 0.0001 && std::abs((*i).z) < 0.0001)
      {
        cloud->points.erase(i);
      }
    }
  }
  ...
  void removeStatisticalOutlier(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr & cloud)
  {
  ...
  void computeCloudNormal(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,
  ...
  void createSphereCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr output_sphere_cloud,
  ...
  void createRectangularCloud(pcl::visualization::PCLVisualizer::Ptr viewer)
  {
    ...
    for(float length_ = 0.0; length_ < length; length_ += 0.001)
    {
     ...
     pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> rgb (rectangle_cloud, 0, 255, 0); //This is blue
