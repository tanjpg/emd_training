  ...
  //void planning_init(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  PCLFunctions::createRectangularCloud(viewer);
  ...
  this->grasp_objects = extractObjects(
    ...
    float(this->get_parameter("point_cloud_params.cloud_normal_radius").as_double()),
  ...
  for(auto object : this->grasp_objects)
  {
    ...
    for(auto gripper : this->end_effectors)
    {
      ...
      grasp_method.grasp_ranks.insert(grasp_method.grasp_ranks.begin(), std::numeric_limits<float>::min());
      ...
      std::cout << "Grasp planning time for " << grasp_method.ee_id <<" : " 
        << std::chrono::duration_cast<std::chrono::milliseconds> (grasp_end - grasp_begin).count() << "[ms]" << std::endl;
    ...
    // for(int method_itr = 0; method_itr < static_cast<int>(object->grasp_target.grasp_methods.size()); method_itr++)
    // {
    //   for(int pose_itr = 0; pose_itr < static_cast<int>(object->grasp_target.grasp_methods[method_itr].grasp_poses.size()); pose_itr++)
    //   {
    //     object->grasp_target.target_pose.pose.position.z += 0.1;
    //     object->grasp_target.target_pose.pose.position.y -= 0.1;
    //     object->grasp_target.grasp_methods[method_itr].grasp_poses[pose_itr].pose.position.z += 0.1;
    //     object->grasp_target.grasp_methods[method_itr].grasp_poses[pose_itr].pose.position.y -= 0.1;
    ...
    std::cout << "Grasp planning time for object: " << std::chrono::duration_cast<std::chrono::milliseconds> (end - begin).count() << "[ms]" << std::endl;
    std::cout << "Visualize the grasps: " <<std::endl;
    grasp_task.grasp_targets.push_back(object->grasp_target);
  }
...
void GraspScene::getCameraPosition()
{
  ...
  float cos_world_table = std::abs((table_normal_vector.dot(worldZVector)) / 
    (table_normal_vector.norm() * worldZVector.norm()));

  std::cout << table_normal_vector(0) << " , " << table_normal_vector(1) << " , " << table_normal_vector(2) << std::endl;
...
std::vector<std::shared_ptr<GraspObject>> GraspScene::extractObjects(std::string camera_frame,
                      ...
                      int min_cluster_size)
  {
    ...
    else
    {
      ...
      for (it = clusterIndices.begin(); it != clusterIndices.end(); ++it) 
      {
        ...
        std::shared_ptr<GraspObject> object = std::make_shared<GraspObject>(camera_frame, objectCloud, centroid);
...
{
  std::vector< std::string > end_effector_array = this->get_parameter("end_effectors.end_effector_names").as_string_array();
  for(std::string end_effector: end_effector_array)    
  {
    std::string end_effector_type = this->get_parameter("end_effectors." + end_effector + ".type").as_string();
    if(end_effector_type.compare("finger") == 0)
    {
      ...
      FingerGripper gripper(
        ...
        float(this->get_parameter("end_effectors." + end_effector + ".distance_between_fingers_1").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".distance_between_fingers_2").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".finger_thickness").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".gripper_stroke").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.voxel_size").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.grasp_rank_weight_1").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.grasp_rank_weight_2").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.grasp_plane_dist_limit").as_double()),          
        float(this->get_parameter("point_cloud_params.cloud_normal_radius").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.world_x_angle_threshold").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.world_y_angle_threshold").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.world_z_angle_threshold").as_double())
        );
    ...
    else if(end_effector_type.compare("suction") == 0)
    {
      ...
      SuctionGripper gripper(
        ...
        float(this->get_parameter("end_effectors." + end_effector + ".dist_between_cups_length").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".dist_between_cups_breadth").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".cup_radius").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".cup_height").as_double()),
        this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.num_sample_along_axis").as_int(),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.search_resolution").as_double()),
        this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.search_angle_resolution").as_int(),
        float(this->get_parameter("point_cloud_params.cloud_normal_radius").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.weights.curvature").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.weights.grasp_distance_to_center").as_double()),
        float(this->get_parameter("end_effectors." + end_effector + ".grasp_planning_params.weights.number_contact_points").as_double()));
...
void GraspScene::processPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  PCLFunctions::passthroughFilter(this->cloud,
  ...
  PCLFunctions::removeStatisticalOutlier(this->cloud);
  PCLFunctions::voxelizeCloud<pcl::PointCloud<pcl::PointXYZRGB>::Ptr, pcl::VoxelGrid<pcl::PointXYZRGB>>(this->cloud, 0.005, this->org_cloud);
  PCLFunctions::planeSegmentation(this->cloud, this->cloud_plane_removed, this->cloud_table, this->segmentation_max_iterations, segmentation_distance_threshold);
}
...
void GraspScene::createWorldCollision(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  this->world_collision_object = FCLFunctions::createCollisionObjectFromPointCloudRGB(this->org_cloud, sensor_origin, 0.01);
