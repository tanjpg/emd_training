...
class SuctionGripper : public EndEffector
{
  ...
  /*! \brief User Defined: Number of grasp samples to be sampled along the axis for each search iteration*/
  const int num_sample_along_axis;
  /*! \brief User Defined: Step size for each search iteration*/
  const float search_resolution;
  /*! \brief User Defined: Angle resolution for each search iteration. Default is 3 for a single 90 degree angle iteration. Min 3, Max 9. */
  ...
  /*! \brief User Defined: Weights to determine importance of the number of contact points. Default is 1.0 */
  ...
  /*! \brief Maximum distance from the center of grasp to the object center, comparing all the grasp samples sampled */
  ...
  bool getCupContactCloud(pcl::PointXYZRGB contact_point,
    float radius, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_input,
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_output);

  std::vector<int> getCupContactIndices(pcl::PointXYZRGB contact_point,
    float radius, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_input);

  pcl::PointXYZRGB findHighestPoint(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud);

  void getStartingPlane(pcl::ModelCoefficients::Ptr plane_coefficients,
  ...
  void projectCloudToPlane(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud,
    pcl::ModelCoefficients::Ptr plane_coefficients, 
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr projected_cloud);

  std::vector<int> createDiskFromCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud,
    pcl::PointCloud<pcl::PointNormal>::Ptr sliced_cloud_normal,
  ...
  void getAllGraspRanks(emd_msgs::msg::GraspMethod *grasp_method, std::shared_ptr<GraspObject> object);

  geometry_msgs::msg::PoseStamped getGraspPose(std::shared_ptr<suctionCupArray> grasp, std::shared_ptr<GraspObject> object);
