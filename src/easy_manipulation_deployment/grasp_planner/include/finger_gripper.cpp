...
void FingerGripper::planGrasps(
  ...
  std::vector<std::shared_ptr<multiFingerGripper>> valid_open_gripper_configs = getAllGripperConfigs(object, world_collision_object);
...
void FingerGripper::getCuttingPlanes(const std::shared_ptr<GraspObject> object)
{
  ...
  pcl::PointXYZ gradient_vector = pcl::PointXYZ(object->axis(0) - object->centerpoint(0), 
  ...
  float d = -((a * object->centerpoint(0)) + (b * object->centerpoint(1)) +(c * object->centerpoint(2)));
  ...
  /*! \brief If any of the sides have odd numbered fingers, we include the center plane in consideration for that side. */
  ...
  if(both_sides_even || both_sides_odd)
  {
    ...
    {
      float initial_gap_1 = (both_sides_even? this->distance_between_fingers_1/2 : this->distance_between_fingers_1);
      float initial_gap_2 = (both_sides_even? this->distance_between_fingers_2/2 : this->distance_between_fingers_2);
      ...
      addCuttingPlanes(object->centerpoint, grasp_plane_vector, num_itr_1, num_itr_2,initial_gap_1, initial_gap_2);
    }
   
  }
  else /*! \brief If both sides are different, the spacing for planes is not consistent, we need to create planes separately */
  {
  ...
  //PCLFunctions::computeCloudNormal(object->cloud, object->cloud_normal, this->cloud_normal_radius);
}
  ...
  const bool both_sides_even)
{
  ...
  float initial_gap = (both_sides_even? this->distance_between_fingers_1/2 : this->distance_between_fingers_1);
  ...
  {
    float gap;
    gap = (updown_toggle == 0? 1:-1) * (initial_gap + (row > 0? (row-1):0) * this->distance_between_fingers_1);
  ...
  float initial_gap_1, float initial_gap_2)
{
  ...
  {
    float gap;
    gap = (updown_toggle_1 == 0? 1:-1) * (initial_gap_1 + side_1 * this->distance_between_fingers_1);
    int plane_index = checkPlaneExists(gap);
    if(plane_index >= 0)
    {
      this->plane_1_index.push_back(plane_index);
  ...
  {
    float gap;
    gap = (updown_toggle_2 == 0? 1:-1) * (initial_gap_2 + side_2 * this->distance_between_fingers_2);
    int plane_index = checkPlaneExists(gap);
    if(plane_index >= 0)
    {
      this->plane_2_index.push_back(plane_index);
...
int FingerGripper::checkPlaneExists(float dist)
{
  ...
  std::vector<float>::iterator it = std::find(this->cutting_plane_distances.begin(), this->cutting_plane_distances.end(), dist);
...
void FingerGripper::addPlane(float dist, Eigen::Vector4f centerpoint , Eigen::Vector4f plane_vector, bool inside_1, bool inside_2)
{
  ...
  Eigen::Vector3f point_on_plane(centerpoint(0) + dist * plane_normal_norm(0), centerpoint(1) + dist * plane_normal_norm(1), centerpoint(2) + dist * plane_normal_norm(2));
  ...
  grasp_sample.plane_eigen(3) = grasp_sample.plane->values[3] = -((plane_vector(0) * point_on_plane(0)) + (plane_vector(1) * point_on_plane(1)) +(plane_vector(2) * point_on_plane(2)));
...
{
  pcl::SampleConsensusModelPlane<pcl::PointNormal>::Ptr planeSAC(new pcl::SampleConsensusModelPlane<pcl::PointNormal>(object->cloud_normal));
  for(auto &sample : this->grasp_samples)
  {
    Eigen::Vector4f plane_vector(sample->plane->values[0], sample->plane->values[1], sample->plane->values[2], sample->plane->values[3]);
    pcl::PointIndices::Ptr grasp_plane_indices(new pcl::PointIndices);
    planeSAC->selectWithinDistance(plane_vector, this->grasp_plane_dist_limit, grasp_plane_indices->indices);
    PCLFunctions::extractInliersCloud<pcl::PointCloud<pcl::PointNormal>::Ptr, 
                        pcl::ExtractIndices<pcl::PointNormal>>(object->cloud_normal,
...
void FingerGripper::getInitialSampleCloud(std::shared_ptr<GraspObject> object)
{
    ...
    std::shared_ptr<GraspObject> &object)-> void
  {
    if(sample->plane_intersects_object)
    {
      PCLFunctions::getClosestPointsByRadius(sample->grasp_plane_ncloud->points[sample->sample_side_1->start_index], this->finger_thickness, object->cloud, object->cloud_normal,
        sample->sample_side_1->finger_cloud, sample->sample_side_1->finger_ncloud);

      PCLFunctions::getClosestPointsByRadius(sample->grasp_plane_ncloud->points[sample->sample_side_2->start_index], this->finger_thickness, object->cloud, object->cloud_normal,
...
void FingerGripper::voxelizeSampleCloud()
{
    ...
    std::shared_ptr<graspPlaneSample> &sample)-> void
  {
    ...
    {
      PCLFunctions::voxelizeCloud<pcl::PointCloud<pcl::PointNormal>::Ptr, pcl::VoxelGrid<pcl::PointNormal>>(
    ...
    pcl::PointNormal &centroid_point)-> void
  {
    ...
    std::shared_ptr<graspPlaneSample> &sample)-> void
    {
      ...
      float grasp_plane_dist = PCLFunctions::normalize(
        ...
        sample->sample_side_1->grasp_plane_dist_min, sample->sample_side_1->grasp_plane_dist_max);
      float curvature = PCLFunctions::normalize(point.curvature,
        sample->sample_side_1->curvature_min, sample->sample_side_1->curvature_max);
      singleFinger finger(point, centroid_dist, grasp_plane_dist, curvature, sample->plane_index);
      static std::mutex mutex;
      std::lock_guard<std::mutex> lock(mutex);
    ...
    std::shared_ptr<graspPlaneSample> &sample)-> void
    {
      ...
      float grasp_plane_dist = PCLFunctions::normalize(
        PCLFunctions::pointToPlane(sample->plane_eigen, point),
        sample->sample_side_2->grasp_plane_dist_min, sample->sample_side_2->grasp_plane_dist_max);
      float curvature = PCLFunctions::normalize(point.curvature,
...
void FingerGripper::getGripperClusters()
{
  for(size_t i = 0 ; i < this->grasp_samples.size(); i++)
  {
    ...
    if(this->grasp_samples[i] -> plane_intersects_object)
    {
      ...
      if(std::find(this->plane_1_index.begin(), this->plane_1_index.end(), i) != this->plane_1_index.end())
      {
      ...
      if(std::find(this->plane_2_index.begin(), this->plane_2_index.end(), i) != this->plane_2_index.end())
      {
  ...
  if(this->grasp_samples[0]->plane_intersects_object)
  {
    for(auto & finger_sample_1 : this->grasp_samples[0]->sample_side_1->finger_samples)
    {
      for(auto & finger_sample_2 : this->grasp_samples[0]->sample_side_2->finger_samples)
      {
        ...
        Eigen::Vector3f centerpoint_side1_vector(finger_sample_1->finger_point.x , finger_sample_1->finger_point.y, finger_sample_1->finger_point.z);
        Eigen::Vector3f centerpoint_side2_vector(finger_sample_2->finger_point.x , finger_sample_2->finger_point.y, finger_sample_2->finger_point.z);
        // Get the vector representing the grasping direction
        Eigen::Vector3f grasp_direction = Eigen::ParametrizedLine<float,3>::Through(centerpoint_side1_vector, centerpoint_side2_vector).direction();
        float grasp_plane_angle_cos_ = 1e-8 + std::abs((grasp_direction.dot(this->center_cutting_plane_normal)) /
        ...
        //TEST
        pcl::PointXYZ parallel_object_vector = pcl::PointXYZ(object->eigenvectors.col(0)(0) - object->centerpoint(0), 
          object->eigenvectors.col(0)(1) - object->centerpoint(1), object->eigenvectors.col(0)(2) - object->centerpoint(2));
        ...
        parallel_object_plane->values[3] =  -((parallel_object_vector.x * point_on_plane(0)) +
          (parallel_object_vector.y * point_on_plane(1)) + (parallel_object_vector.z * point_on_plane(2)));
        Eigen::Vector3f perpendicular_grasp_direction = getPerpendicularVectorInPlane(grasp_direction, parallel_object_plane);
        Eigen::Vector3f perpendicular_grasp_direction_normalized = perpendicular_grasp_direction/ perpendicular_grasp_direction.norm();
        //END TEST

        // Get End effector finger clouds
        Eigen::Vector3f grasp_direction_normalized = grasp_direction/grasp_direction.norm();
        // Get the centerpoint vector of the grasp
        Eigen::Vector3f side1_2_centerpoint_vector((centerpoint_side1_vector(0) + centerpoint_side2_vector(0))/2,
          (centerpoint_side1_vector(1) + centerpoint_side2_vector(1))/2,
          (centerpoint_side1_vector(2) + centerpoint_side2_vector(2))/2);
        
        //Get the initial finger positions in the open configuration from the center plane. this may or may not be used depending
        // on number of fingers
        Eigen::Vector3f open_center_finger_1 = side1_2_centerpoint_vector - (this->gripper_stroke/2) * grasp_direction_normalized;
        Eigen::Vector3f open_center_finger_2 = side1_2_centerpoint_vector + (this->gripper_stroke/2) * grasp_direction_normalized;
        std::shared_ptr<multiFingerGripper> gripper_sample = generateGripperOpenConfig(
            ...
            open_center_finger_1, open_center_finger_2, perpendicular_grasp_direction_normalized, grasp_direction);
  ...
  Eigen::Vector3f plane_normal_normalized, Eigen::Vector3f grasp_direction)
{
  ...
  float initial_gap_1 = (is_even_1? this->distance_between_fingers_1/2 : this->distance_between_fingers_1);
  float initial_gap_2 = (is_even_2? this->distance_between_fingers_2/2 : this->distance_between_fingers_2);
  ...
  for(int side_1 = 0, updown_toggle_1 = 1; side_1 < num_itr_1; side_1 += updown_toggle_1^=1)
  {
    ...
    for(size_t plane_index_ = 0 ; plane_index_ < this->cutting_plane_distances.size(); plane_index_++)
    {
    ...
    // Now that we find the plane, we need to find the point near that plane. this is done using the kdtree search function
    ...
    // std::cout << "Open Point at: " << finger_1_point.x << "," << finger_1_point.y << "," << finger_1_point.z << std::endl;
    ...
    if(kdtree.nearestKSearch (finger_1_point, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 )
    {
      ...
      // finger_1_contactpoint = this->grasp_samples[plane_index].sample_side_1.finger_nvoxel->points[pointIdxNKNSearch[0]];
      // std::cout << "Real Point at: " << finger_1_contactpoint.x << "," << finger_1_contactpoint.y << "," << finger_1_contactpoint.z << std::endl;
      gripper.closed_fingers_1.push_back(this->grasp_samples[plane_index]->sample_side_1->finger_samples[pointIdxNKNSearch[0]]);
  ...
  {
    float gap2;
    gap2 = (updown_toggle_2 == 0? 1:-1) * (initial_gap_2 + side_2 * this->distance_between_fingers_2);
    ...
    for(size_t plane_index_2_ = 0 ; plane_index_2_ < this->cutting_plane_distances.size(); plane_index_2_++)
    {
    ...
    if ( kdtree.nearestKSearch (finger_2_point, K, pointIdxNKNSearch, pointNKNSquaredDistance) > 0 )
    {
      ...
      // finger_2_contactpoint = this->grasp_samples[plane_index_2].sample_side_2.finger_nvoxel->points[pointIdxNKNSearch[0]];
      gripper.closed_fingers_2.push_back(this->grasp_samples[plane_index_2]->sample_side_2->finger_samples[pointIdxNKNSearch[0]]);
  ...
  for(auto & finger : gripper.closed_fingers_1)
  {
    ...
    finger->angle_cos= 1e-8 + 
      std::abs((grasp_direction.dot(finger_normal)) /
  ...
  const std::shared_ptr<fcl::CollisionObject<float>> world_collision_object)
{
  ...
  std::shared_ptr<fcl::CollisionObject<float>> finger_ptr = std::make_shared<fcl::CollisionObject<float>>(finger);

  fcl::CollisionRequest<float> request;
  request.enable_contact = true;
    // num_max_contacts_ = 1, //The maximum number of contacts will return. 
	  // enable_contact_ = false, //whether the contact information (normal, penetration depth and contact position) will return 
  ...
  emd_msgs::msg::GraspMethod *grasp_method)
{
  ...
  for(auto gripper : input_vector)
  {
    ...
    size_t rank;
    for (rank = 0, grasps_it = grasp_method->grasp_poses.begin(), contacts_it = sorted_gripper_ranks.begin();
...
void FingerGripper::getGripperRank(std::shared_ptr<multiFingerGripper> gripper)
{
  ...
  float rank_1 = (gripper->closed_fingers_1.size() + gripper->closed_fingers_2.size()) - grasp_plane_dist_sum
    - (gripper->grasp_plane_angle_cos - 0.2) * 10.0;
    //More to add for rank 2
  float rank_2 = (gripper->closed_fingers_1.size() + gripper->closed_fingers_2.size()) - curvature_sum;
 ...
 ******************************************************************************/
void FingerGripper::getGraspPose(std::shared_ptr<multiFingerGripper> gripper ,std::shared_ptr<GraspObject> object)
{
  ...
  tf2::Matrix3x3 rotation_matrix(object->affine_matrix(0,0),object->affine_matrix(1,0),object->affine_matrix(2,0),
  ...
  result_pose.header.stamp.sec = std::chrono::duration_cast<std::chrono::seconds>(clock.time_since_epoch()).count();
 ...
 ******************************************************************************/
Eigen::Vector3f FingerGripper::getPerpendicularVectorInPlane(Eigen::Vector3f target_vector, pcl::ModelCoefficients::Ptr plane)
{
  Eigen::Vector4f plane_vector(plane->values[0], plane->values[1], plane->values[2], plane->values[3]);
  Eigen::Vector3f plane_normal_vector(plane->values[0], plane->values[1], plane->values[2]);
  float z_bar = plane_normal_vector(0) * target_vector(2) - target_vector(0) * plane_normal_vector(2);
  float y_bar = plane_normal_vector(0) * target_vector(1) - target_vector(0) * plane_normal_vector(1);
  float y_numerator = (-plane_vector(3) * plane_normal_vector(0) * z_bar);
  float y_denominator = (plane_vector(0) * plane_normal_vector(2) * y_bar) - (plane_normal_vector(1) * z_bar)
    + (plane_vector(1) * plane_normal_vector(0) * z_bar) - (plane_vector(2) * plane_normal_vector(0) * y_bar);
  float y_temp = y_numerator / y_denominator;
  float x_temp = y_temp * (((plane_normal_vector(2) * y_bar)/(plane_normal_vector(0) * z_bar)) - (plane_normal_vector(1)/plane_normal_vector(0)));
...
void FingerGripper::visualizeGrasps(pcl::visualization::PCLVisualizer::Ptr viewer)
{
  ...
  //   pcl::PointXYZ check_vector = pcl::PointXYZ(object->eigenvectors.col(0)(0) - object->centerpoint(0), 
  //   object->eigenvectors.col(0)(1) - object->centerpoint(1), object->eigenvectors.col(0)(2) - object->centerpoint(2));
  //   viewer->addPlane(*(this->grasp_samples[plane_1_index[i]]->plane), "plane_" + std::to_string(i), 0);
  // }
  // viewer->spin();
  // viewer->close();
  // viewer->removeAllShapes();
  ...
  //   pcl::PointXYZ check_vector = pcl::PointXYZ(object->eigenvectors.col(0)(0) - object->centerpoint(0), 
  //   object->eigenvectors.col(0)(1) - object->centerpoint(1), object->eigenvectors.col(0)(2) - object->centerpoint(2));
  //   viewer->addPlane(*(this->grasp_samples[plane_2_index[i]]->plane), "plane_" + std::to_string(i), 0);
  // }
  // viewer->spin();
  // viewer->close();
  // viewer->removeAllShapes();
  ...
  // for(size_t i = 0 ; i < this->grasp_samples.size(); i++)
  // {
  //   viewer->addPointCloud<pcl::PointNormal>(this->grasp_samples[i]->grasp_plane_ncloud,"cloud_" + std::to_string(i));
  // }
  // viewer->spin();
  // viewer->close();
  // viewer->removeAllShapes();
  ...
  //       viewer->addSphere(this->grasp_samples[i]->grasp_plane_ncloud->points[index], 0.005, "finger_point_1" + std::to_string(i));
  //     }
  //     if(std::find(this->plane_2_index.begin(), this->plane_2_index.end(), i) != this->plane_2_index.end())
  //     {
  //       int index = this->grasp_samples[i]->sample_side_2->start_index;
  //       viewer->addSphere(this->grasp_samples[i]->grasp_plane_ncloud->points[index], 0.005, "finger_point_2" + std::to_string(i));
  //     }
  //   }
  ...
  // for(size_t i = 0 ; i < this->grasp_samples.size(); i++)
  // {
  //   viewer->addPointCloud<pcl::PointXYZRGB>(this->grasp_samples[i].sample_side_1.finger_cloud,"cloud1_" + std::to_string(i));
  //   viewer->addPointCloud<pcl::PointXYZRGB>(this->grasp_samples[i].sample_side_2.finger_cloud,"cloud2_" + std::to_string(i));
  // }
  ...
  // for(size_t i = 0 ; i < this->grasp_samples.size(); i++)
  // {
  //   viewer->addPointCloud<pcl::PointNormal>(this->grasp_samples[i].sample_side_1.finger_ncloud,"cloud1_" + std::to_string(i));
  //   viewer->addPointCloud<pcl::PointNormal>(this->grasp_samples[i].sample_side_2.finger_ncloud,"cloud2_" + std::to_string(i));  
  // }
  // viewer->spin();
  // viewer->close();
  // viewer->removeAllShapes();
  ...
  // for(size_t i = 0 ; i < this->grasp_samples.size(); i++)
  // {
  //   viewer->addPointCloud<pcl::PointNormal>(this->grasp_samples[i].sample_side_1.finger_nvoxel,"cloud1_" + std::to_string(i));
  //   viewer->addPointCloud<pcl::PointNormal>(this->grasp_samples[i].sample_side_2.finger_nvoxel,"cloud2_" + std::to_string(i));
  // }
  ...
  // std::cout << "Valid open gripper configs size: " << valid_open_gripper_configs.size() << std::endl;
  ...
  for(auto const & multigripper : this->sorted_gripper_configs)
  {
    ...
    {
      viewer->addSphere(multigripper->closed_fingers_1[cs_1]->finger_point, 0.01, 1.0, 0, 0, "finger_point_cs_1" + std::to_string(cs_1));
    }
    ...
    {
      viewer->addSphere(multigripper->closed_fingers_2[cs_2]->finger_point, 0.01, 1.0, 0, 0, "finger_point_cs_2" + std::to_string(cs_2));
    }
...
//       // Get the vector representing the gripper direction
//       Eigen::Vector3f grasp_direction = Eigen::ParametrizedLine<float,3>::Through(curr1_vector, curr2_vector).direction();

//       // Get End effector finger clouds
//       Eigen::Vector3f grasp_direction_norm = grasp_direction/grasp_direction.norm();
//       Eigen::Vector3f curr1_2_centerpoint_vector((curr1_point.x + curr2_point.x)/2,(curr1_point.y + curr2_point.y)/2, (curr1_point.z + curr2_point.z)/2);
//       Eigen::Vector3f finger1_point = curr1_2_centerpoint_vector + (this->gripper_stroke/2) * grasp_direction_norm;
//       Eigen::Vector3f finger2_point = curr1_2_centerpoint_vector - (this->gripper_stroke/2) * grasp_direction_norm;
...
//       // PCLFunctions::createSphereCloud(finger1_cloud, finger1_point, this->finger_thickness/2, 20);
//       // PCLFunctions::createSphereCloud(finger2_cloud, finger2_point, this->finger_thickness/2, 20);
...
//       // if (CheckFingerCollision(fingers[0], world_collision_object) || CheckFingerCollision(fingers[1], world_collision_object))
//       // {
...
//       // if (CheckFingerCollision(world_collision_object, finger_1_ptr) || CheckFingerCollision(world_collision_object, finger_2_ptr))
//       // {
//       //   // continue; //Finger collides
//       //   return;
//       // }
//       float grasp_plane_angle_cos = epsilon + std::abs((grasp_direction.dot(this->center_cutting_plane_normal)) /
//         (grasp_direction.norm() * this->center_cutting_plane_normal.norm()));
      
//       // float grasp_plane_angle_cos = epsilon + std::abs((grasp_direction.dot(object->grasp_plane_normal)) /
...
//       //Insert the rank and grasp into the vectors in object (Might remove, takes time to iterate)
//       std::lock_guard<std::mutex> lock(obj_2_mutex);
//       int rank;
//       std::vector<geometry_msgs::msg::PoseStamped>::iterator grasps_it;
//       std::vector<GraspContacts>::iterator contacts_it;
//       for (rank = 0, grasps_it = grasp_method->grasp_poses.begin(), contacts_it = grasp_contacts.begin(); rank < static_cast<int>(grasp_method->grasp_ranks.size()); 
...
//           grasp_method->grasp_poses.insert(grasps_it, getGraspPose(new_grasp, object->object_frame, object));
...
//     //If the first point centroid distance is lesser than the initial first point centroid distance, OR
//     // if the initial distance between grasp points is wider than the current distance between grasp points
...
//   float rank_1 = 2.0 - point1_grasp_plane_dist - point2_grasp_plane_dist - (grasp_plane_angle_cos - 0.20) * 10.0;
