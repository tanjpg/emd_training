...
struct singleFinger{
  ...
  singleFinger(pcl::PointNormal finger_point_,
...
struct multiFingerGripper{
  ...
  multiFingerGripper(std::shared_ptr<singleFinger>base_point_1_, std::shared_ptr<singleFinger> base_point_2_):
...
class FingerGripper : public EndEffector
{
  ...
  void getBestGrasps(const std::shared_ptr<GraspObject> object,
  ...
  void getGraspPose(std::shared_ptr<multiFingerGripper> gripper ,const std::shared_ptr<GraspObject> object);
  void planGrasps(
  ...
  void addCuttingPlanesEqualAligned(const Eigen::Vector4f centerpoint, const Eigen::Vector4f plane_vector, const bool both_sides_even);
  void addCuttingPlanes(const Eigen::Vector4f &centerpoint, const Eigen::Vector4f &plane_vector,
  int num_itr_1, int num_itr_2, float initial_gap_1, float initial_gap_2);

  int checkPlaneExists(float dist);
  void addPlane(float dist, Eigen::Vector4f centerpoint , Eigen::Vector4f plane_vector, bool inside_1, bool inside_2);

  void getGripperClusters();
  void getGripperSamplesFromClusters();
  void getFingerSamples(std::shared_ptr<GraspObject> object);
  std::vector<std::shared_ptr<multiFingerGripper>> getAllGripperConfigs(std::shared_ptr<GraspObject> object, std::shared_ptr<fcl::CollisionObject<float>> world_collision_object);
  std::shared_ptr<multiFingerGripper> generateGripperOpenConfig(
    std::shared_ptr<fcl::CollisionObject<float>> world_collision_object, std::shared_ptr<singleFinger> closed_center_finger_1,
    std::shared_ptr<singleFinger> closed_center_finger_2, Eigen::Vector3f open_center_finger_1,
    Eigen::Vector3f open_center_finger_2, Eigen::Vector3f plane_normal_normalized, Eigen::Vector3f grasp_direction);
  bool checkFingerCollision(Eigen::Vector3f finger_point,
    std::shared_ptr<fcl::CollisionObject<float>> world_collision_object);
  ...
  Eigen::Vector3f getPerpendicularVectorInPlane(Eigen::Vector3f target_vector, pcl::ModelCoefficients::Ptr plane);
  void getGripperRank(std::shared_ptr<multiFingerGripper> gripper);
  std::vector<std::shared_ptr<multiFingerGripper>> getAllRanks(std::vector<std::shared_ptr<multiFingerGripper>> input_vector, emd_msgs::msg::GraspMethod *grasp_method);
