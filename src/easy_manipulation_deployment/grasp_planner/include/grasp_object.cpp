...
GraspObject::GraspObject(std::string object_frame_,
...
GraspObject::GraspObject(std::string object_name_, std::string object_frame_,
              pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_, 
              Eigen::Vector4f centerpoint_) :
              object_frame(object_frame_),
              cloud(new pcl::PointCloud<pcl::PointXYZRGB>()),
...
{
  viewer->addCube(this->bboxTransform, this->bboxQuaternion, 
...
void GraspObject::get_object_bb()
{
  ...
  this->axis = Eigen::Vector3f(this->eigenvectors.col(2)(0), 
  ...
  projectionTransform.block<3,1>(0,3) = -1.f * (projectionTransform.block<3,3>(0,0) * this->centerpoint.head<3>());
  ...
  const Eigen::Vector3f meanDiagonal = 0.5f*(this->maxPoint.getVector3fMap() + this->minPoint.getVector3fMap());
...
void GraspObject::get_object_world_angles()
{
  ...
  this->objectWorldCosX = std::abs((this->axis.dot(worldXVector)) / 
      (this->axis.norm() * worldXVector.norm()));
  this->objectWorldCosY = std::abs((this->axis.dot(worldYVector)) / 
    (this->axis.norm() * worldYVector.norm()));
  this->objectWorldCosZ = std::abs((this->axis.dot(worldZVector)) / 
...
geometry_msgs::msg::PoseStamped GraspObject::getObjectPose(std::string pose_frame)
{
  ...
  midPointRotation << this->affine_matrix(0,0), this->affine_matrix(0,1), this->affine_matrix(0,2), 
  ...
  // result_pose.header.stamp.nsec = std::chrono::duration_cast<std::chrono::nanoseconds>(clock.time_since_epoch()).count();

  return result_pose;
}

shape_msgs::msg::SolidPrimitive GraspObject::getObjectShape() //This function is highly dependant on what the camera axis is.
{
