  ...
  pcl::visualization::PCLVisualizer::Ptr viewer)
{
  ...
  pcl::PointXYZRGB object_top_point = findHighestPoint(object->cloud); // Get highest point of the object to begin grasp search



  // std::chrono::steady_clock::time_point getAllPossibleGraspsStart = std::chrono::steady_clock::now(); 
  getAllPossibleGrasps(object, object_center, object_top_point, viewer);
  // std::chrono::steady_clock::time_point getAllPossibleGraspsEnd = std::chrono::steady_clock::now();
  // std::cout << "Grasp planning time for getAllPossibleGrasps "
        // << std::chrono::duration_cast<std::chrono::milliseconds> (getAllPossibleGraspsEnd - getAllPossibleGraspsStart).count() << "[ms]" << std::endl;
  ...
  // std::cout << "Grasp planning time for getAllGraspRanks "
        // << std::chrono::duration_cast<std::chrono::milliseconds> (getAllGraspRanksEnd - getAllGraspRanksStart).count() << "[ms]" << std::endl;
...
void SuctionGripper::getAllPossibleGrasps(
  ...
  pcl::visualization::PCLVisualizer::Ptr viewer){
      ...
      )-> void
    {
      ...
      pcl::PointCloud<pcl::PointNormal>::Ptr sliced_cloud_normal (new pcl::PointCloud<pcl::PointNormal>);
      ...
      //getSlicedCloud(object->cloud, slice_limit, 0, sliced_cloud);
       ...
       /*! \brief A sliced cloud is created to account for noise, so we take a range of z values and assume them to be in the same height*/
      getSlicedCloud(object->cloud, object->cloud_normal, slice_limit, 0, sliced_cloud, sliced_cloud_normal);
      /*! \brief We then make them part of the same plane throguh projection*/
      projectCloudToPlane(sliced_cloud, plane, projected_cloud);
      /*! \brief Get the center index of the sliced cloud, which may not necessarily be the center of the object cloud*/
      int centroid_index = getCentroidIndex(projected_cloud);
      //viewer->addPointCloud<pcl::PointXYZRGB>(projected_cloud, "projected_cloud");
      //PCLFunctions::computeCloudNormal(sliced_cloud, sliced_cloud_normal, this->cloud_normal_radius);


      
      for(int i=2, angle = 2,toggle=1; i < this->search_angle_resolution; i+= toggle^=1) // Iterate at different angles to search for best possible grasp
      {                
      angle = (toggle==1? i:-i);
      
      for(int j = 0, k = 0; j < this->num_sample_along_axis; j++, k^=1)
        {
         float object_max_dim = std::max(std::max(object->maxPoint.x - object->minPoint.x,
                                            object->maxPoint.y - object->minPoint.y),
                                        object->maxPoint.z - object->minPoint.z);
  
        static std::mutex mutex_2;
        float offset = this->search_resolution * (k == 0 ? j : -j); // Iterate between the positive and negative sides
        pcl::PointXYZ sample_gripper_center;
        sample_gripper_center.x = projected_cloud->points[centroid_index].x + (object->eigenvectors.col(2)(0)) * offset;
        sample_gripper_center.y = projected_cloud->points[centroid_index].y + (object->eigenvectors.col(2)(1)) * offset;
        ...
        Eigen::Vector3f grasp_direction; 
        grasp_direction(0) = object->eigenvectors.col(1)(0) * cos(PI/angle) - object->eigenvectors.col(1)(1) * sin(PI/angle);
        grasp_direction(1) = object->eigenvectors.col(1)(0) * sin(PI/angle) + object->eigenvectors.col(1)(1) * cos(PI/angle);
        grasp_direction(2) = object->eigenvectors.col(1)(2);
        Eigen::Vector3f grasp_direction_normalized = grasp_direction/grasp_direction.norm();

        Eigen::Vector3f object_direction; 
        object_direction(0) = object->eigenvectors.col(2)(0) * cos(PI/angle) - object->eigenvectors.col(2)(1) * sin(PI/angle);
        object_direction(1) = object->eigenvectors.col(2)(0) * sin(PI/angle) + object->eigenvectors.col(2)(1) * cos(PI/angle);
        object_direction(2) = object->eigenvectors.col(2)(2);
        Eigen::Vector3f object_direction_normalized = object_direction/object_direction.norm();
        ...
        float initial_gap_breadth = this->dist_between_cups_breadth / (1.0 + (this->num_cups_breadth % 2 == 0? 1 : 0));
        float initial_gap_length = this->dist_between_cups_length / (1.0 + (this->num_cups_length % 2 == 0 ? 1 : 0));
        ...
        for(int row = 0, row_updown_toggle = 0; row < (this->breadth_dim < this->length_dim? num_itr_breadth : num_itr_length); row += row_updown_toggle ^=1)
        {
          std::cout << "row: " << row << std::endl;
          std::cout << "row toggle: " << row_updown_toggle << std::endl;
          float row_gap;   
          if(this->breadth_dim < this->length_dim)
          {
            ...
            row_gap = ((row==0? 0 : 1) * initial_gap_breadth + (row > 0? (row-1):0)* this->dist_between_cups_breadth);  
          }
          else{
            ...
            row_gap = ((row==0? 0 : 1) * initial_gap_length + (row > 0? (row-1):0)* this->dist_between_cups_length); 
          }
                 
          Eigen::Vector3f row_cen_point = grasp_centerpoint + (row_updown_toggle == 0 ? row_gap : -row_gap) * grasp_direction_normalized;
          std::vector<std::shared_ptr<singleSuctionCup>> temp_row_array;          

          for(int col = 0, col_updown_toggle = 0; col < (this->breadth_dim < this->length_dim? num_itr_length : num_itr_breadth); col+= col_updown_toggle^=1)
          {
            float col_gap;
            if(this->breadth_dim < this->length_dim)
            {
              if(is_even_length && col == 0) // For even numbers, the center axis of the gripper does not contain any gripper
              {
                continue;
              }
              col_gap = ((col==0? 0 : 1) * initial_gap_length + (col > 0? (col-1):0) * this->dist_between_cups_length);
            }
            else{
              if(is_even_breadth && col == 0) // For even numbers, the center axis of the gripper does not contain any gripper
              {
                continue;
              }
              col_gap = ((col==0? 0 : 1) * initial_gap_breadth + (col > 0? (col-1):0) * this->dist_between_cups_breadth); 
            }
            
            //pcl::PointXYZ sample_gripper_center;
            Eigen::Vector3f cup_vector = row_cen_point + (col_updown_toggle == 0 ? col_gap : -col_gap) * object_direction_normalized;
            ...
            //Check how many points of the projected point cloud land on a suction cup of fixed radius.
            float* curvature_sum_ptr;
            float curvature_sum;
            curvature_sum_ptr = &curvature_sum;
            // std::cout << "Curvature: " << *curvature_sum_ptr << std::endl;
            // std::vector<int> contact_points = createDiskFromCloud(projected_cloud, sliced_cloud_normal, cup_point, this->cup_radius
            ...
            num_contact_points = std::round(num_contact_points * (this->num_contact_points_weight * ( 1 - cup_object_center_dist/object_max_dim)));
            ...
            cup.contact_points = num_contact_points;
        ...
        temp_cup_array.center_dist = pcl::geometry::distance(sample_gripper_center, object_center);
        temp_cup_array.average_curvature =  temp_cup_array.total_curvature / temp_cup_array.total_contact_points;
        // std::cout << " Total Curvature: " << temp_cup_array.total_curvature << std::endl;
        // std::cout << " Total Contact points: " << temp_cup_array.total_contact_points << std::endl;
        // std::cout << "Average Curvature: " << temp_cup_array.average_curvature << std::endl;
        updateMaxMinValues(temp_cup_array.total_contact_points, temp_cup_array.average_curvature, temp_cup_array.center_dist); 
        temp_cup_array.grasp_angle = PI/angle;
        std::cout << "TEMP CUP ARRAY GRASP ANGLE: " << temp_cup_array.grasp_angle <<std::endl;
    ...
    for(int i = 0; i < int(round(this->cup_height/0.001)) ; i++)
    {
      //Start async
      futures_1.push_back(
  ...
  if(this->cup_array_samples.size() > 0)
  {
    for(auto suction_cup_array: this->cup_array_samples)
    {
      ...
      for(auto row : suction_cup_array->cup_array)
      {
        for(auto cup : row)
        {
          viewer->addSphere(cup->cup_center, this->cup_radius, "test_point" + std::to_string(counter));
          counter++;
        }
      }
      viewer->spin();
...
void SuctionGripper::getAllGraspRanks(emd_msgs::msg::GraspMethod *grasp_method, std::shared_ptr<GraspObject> object)
{
  ...
  {
    float contact_points_norm = PCLFunctions::normalizeInt(grasp->total_contact_points,
      this->min_contact_points, this->max_contact_points);
    
    float curvature_norm = PCLFunctions::normalize(grasp->average_curvature,
      this->min_curvature, this->max_curvature);
    
    float  grasp_center_norm = PCLFunctions::normalize(grasp->center_dist,
    ...
    if(sorted_grasps.size() == 0 && grasp_method->grasp_ranks.empty() && grasp_method->grasp_poses.empty())
    {
      ...
      int index;
      for (index = 0, pose_it = grasp_method->grasp_poses.begin(), grasp_it = sorted_grasps.begin(); index < static_cast<int>(grasp_method->grasp_ranks.size()); 
...
// Provides the cloud representing the contact with suction gripper (Only for visualization purposes) (CURRENTLY NOT USED)
...
// Provides the cloud point indices. For actual implementation. No need to generate the cloud (CURRENTLY NOT USED)
...
void SuctionGripper::getStartingPlane(pcl::ModelCoefficients::Ptr plane_coefficients,
...
void SuctionGripper::getSlicedCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud,
...
void SuctionGripper::projectCloudToPlane(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud,
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr projected_cloud)
{
  ...
  //std::cout << "Grasp planning time for projectCloudToPlane "
  //    << std::chrono::duration_cast<std::chrono::milliseconds> (getSlicedCloud2 - getSlicedCloud1).count() << "[ms]" << std::endl;
}

std::vector<int> SuctionGripper::createDiskFromCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud,
  pcl::PointCloud<pcl::PointNormal>::Ptr sliced_cloud_normal,
...
void SuctionGripper::updateMaxMinValues(int num_contact_points, float average_curvature, float center_dist)
{
  ...
  if(num_contact_points > this->max_contact_points){this->max_contact_points = num_contact_points;}
...
geometry_msgs::msg::PoseStamped SuctionGripper::getGraspPose(std::shared_ptr<suctionCupArray> grasp, std::shared_ptr<GraspObject> object)
{
  ...
  // std::cout << "RESULT POSE POSITION X: " << result_pose.pose.position.x << std::endl;
  // std::cout << "RESULT POSE POSITION Y: " << result_pose.pose.position.y << std::endl;
  // std::cout << "RESULT POSE POSITION Z: " << result_pose.pose.position.z << std::endl;
  //  result_pose.pose.position.x = object->centerpoint(0);
  // result_pose.pose.position.y = object->centerpoint(1);
  // result_pose.pose.position.z = object->centerpoint(2);
  ...
  projectionTransform_test.block<3,1>(0,3) = -1.f * (projectionTransform_test.block<3,3>(0,0) * object->centerpoint.head<3>());
  this->affine_matrix_test = projectionTransform_test;

  Eigen::Matrix3f PointRotation;
  PointRotation << this->affine_matrix_test(0,0), this->affine_matrix_test(0,1), this->affine_matrix_test(0,2), 
                      this->affine_matrix_test(1,0), this->affine_matrix_test(1,1), this->affine_matrix_test(1,2),
                      this->affine_matrix_test(2,0), this->affine_matrix_test(2,1), this->affine_matrix_test(2,2);
  
   
  // Eigen::Matrix3f rotatedEigenVector1 = PointRotation;
  
  // Eigen::Matrix3f rotatedEigenVector1 = rotationMatrix * (AlignedRotationMatrix * object->eigenvectors);
  ...
  // std::cout << "RESULT POSE orientation X: " << result_pose.pose.orientation.x << std::endl;
  // std::cout << "RESULT POSE orientation Y: " << result_pose.pose.orientation.y << std::endl;
  // std::cout << "RESULT POSE orientation Z: " << result_pose.pose.orientation.z << std::endl;
  // std::cout << "RESULT POSE orientation W: " << result_pose.pose.orientation.w << std::endl;
